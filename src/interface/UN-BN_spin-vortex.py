...
# eta = np.where(Z > -1, -Z / 2 + 0.5, 1)  # Parameter used to interpolate between states
# eta = np.where(Z > 1, 0, eta)
eta = np.where(Z < 0, -0.5, cp.sqrt(3) / 2)  # Parameter used to interpolate between states
...
filename = 'UN-BN_new_interface_new'  # Name of file to save data to
