...
psiP2 = cp.sqrt(Tf) * C ** 4
psiP1 = cp.sqrt(Tf) * cp.exp(1j * phi) * 2 * C ** 3 * S
psi0 = cp.sqrt(Tf) * cp.sqrt(6) * cp.exp(2j * phi) * C ** 2 * S ** 2
psiM1 = cp.sqrt(Tf) * 2 * cp.exp(3j * phi) * C * S ** 3
psiM2 = cp.sqrt(Tf) * cp.exp(4j * phi) * S ** 4
...
filename = 'FM_coreless'  # Name of file to save data to
