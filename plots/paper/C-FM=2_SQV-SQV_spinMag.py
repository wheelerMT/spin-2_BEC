...
ax_consty.set_ylim(-y.max(), y.max())
...
plt.savefig('../../../plots/spin-2/paper/C-FM=2_SQV-SQV_spinMag_consty.png', bbox_inches='tight', dpi=200)

# Construct plot at constant z
fig_constz, ax_constz = plt.subplots(1, figsize=(4.2, 3.54))
ax_constz.set_xlim(-x.max(), x.max())
ax_constz.set_ylim(-y.max(), y.max())
ax_constz.set_xlabel(r'$x/\ell$')
ax_constz.set_ylabel(r'$y/\ell$')
z_index = Nz // 2 - 20
constz_plot = ax_constz.contourf(X[:, :, z_index], Y[:, :, z_index], spin_expec[:, :, z_index], levels=200, cmap='jet')
constz_cbar = plt.colorbar(constz_plot, ax=ax_constz, pad=0.01)
constz_cbar.set_ticks([0, 1, 1.99])
constz_cbar.set_ticklabels(['0', '1', '2'])
plt.savefig('../../../plots/spin-2/paper/C-FM=2_SQV-SQV_spinMag_constz.png', bbox_inches='tight', dpi=200)
plt.show()
