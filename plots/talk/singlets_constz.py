...
# Load in data:
data_path = 'frames/10f_C-FM=2_coreless'   # input('Enter file path of data to view: ')
...
# # First row:
# one_spin_plot = grid[0].contourf(X[:, :, z_upper_ind], Y[:, :, z_upper_ind], abs(spin_expec[:, :, z_upper_ind]),
#                                  np.linspace(0, 2.01, 100), cmap='jet')
# one_a20_plot = grid[1].contourf(X[:, :, z_upper_ind], Y[:, :, z_upper_ind], abs(a20[:, :, z_upper_ind]) ** 2,
#                                 np.linspace(0, 0.201, 100), cmap='jet')
# one_a30_plot = grid[2].contourf(X[:, :, z_upper_ind], Y[:, :, z_upper_ind], abs(a30[:, :, z_upper_ind]) ** 2,
#                                 np.linspace(0, 2.01, 100), cmap='jet')

# Second row:
two_spin_plot = grid[0].contourf(X[:, :, z_lower_ind], Y[:, :, z_lower_ind], abs(spin_expec[:, :, z_lower_ind]),
                                   np.linspace(0, 2.01, 100), cmap='jet')
# two_a20_plot = grid[1].contourf(X[:, :, z_lower_ind], Y[:, :, z_lower_ind], abs(a20[:, :, z_lower_ind]) ** 2,
#                                   np.linspace(0, 0.201, 100), cmap='jet')
# two_a30_plot = grid[2].contourf(X[:, :, z_lower_ind], Y[:, :, z_lower_ind], abs(a30[:, :, z_lower_ind]) ** 2,
#                                   np.linspace(0, 2.01, 100), cmap='jet')

# Set colorbars
# for i, contour in enumerate([two_spin_plot]):
#     grid[i * 3].cax.colorbar(contour, ticks=[0, 1, 2])
#     grid[i * 3].cax.toggle_label(True)
# for i, contour in enumerate([two_a20_plot]):
#     grid[(i * 3) + 1].cax.colorbar(contour, ticks=[0, 1 / 5])
#     grid[(i * 3) + 1].cax.toggle_label(True)
# for i, contour in enumerate([two_a30_plot]):
#     grid[(i * 3) + 2].cax.colorbar(contour, ticks=[0, 2])
#     grid[(i * 3) + 2].cax.toggle_label(True)
grid[0].cax.colorbar(two_spin_plot, ticks=[0, 2])
grid[0].cax.toggle_label(True)

plt.tight_layout()
plt.savefig('../../../plots/spin-2/write-up/C-FM=2_coreless_spin_late_cyclic_side.png', bbox_inches="tight")
plt.show()
